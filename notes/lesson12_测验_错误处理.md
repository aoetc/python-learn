...
**7.** 用户输入 `60`，`int(input())` 的结果是？D，我觉得输入‘60’，代表的可能是字符60把，并非数字60
...
**1.** `try` 里放可能出错的代码，`except` 里放出错后的处理。对

**2.** `int("12.5")` 一定成功得到整数 12。错

**3.** 捕获 `ValueError` 以后，程序还可以继续往下跑（取决于你怎么写）。对

**4.** 所有错误都应该用 `except:` 一网打尽，什么都不写类型。错，需要分清错误类型吧

**5.** `finally` 里适合做「一定要做的收尾」（了解即可）。对，解释一下

**6.** 错误处理和递归是同一章必须一起学完的内容。？这种东西就不要出成题目了吧，这个是规划的内容
